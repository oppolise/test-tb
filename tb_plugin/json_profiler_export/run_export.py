...
def process_run_data(logdir: str, cache_dir: str):
    ...
    """
    processed_data = {
        "source_logdir": logdir, # Add this line
        "workers": {},
        "all_recommendations": [],
        "errors": [],
...
def get_runs(logdir: str) -> list[str]:
    """
    Scans the logdir for subdirectories that are considered "runs".
    A subdirectory is a run if it directly contains profiler trace files.
    Returns a list of run names (subdirectory basenames).
    """
    runs = set() # Use a set to avoid duplicates if logdir itself is a run
    if not io.isdir(logdir):
        logger.warning(f"Logdir {logdir} is not a directory or does not exist.")
        return []

    # Check if the logdir itself contains trace files (could be a single run)
    # For now, let's assume runs are always subdirectories as per user's refined request.
    # If logdir itself can be a run, this logic might need adjustment.
    # The user described: "--logdir=/log_1Batch ก็ดูว่ามันมีโฟเดอร็อีกไหมนะ เพื่อที่ว่าอาจะมี 2 โฟเดอร์ที่อยู่ใน log_1Batch"
    # This implies scanning subdirectories of the given logdir.

    for item_name in io.listdir(logdir):
        item_path = io.join(logdir, item_name)
        if io.isdir(item_path):
            # This is a potential run directory (a subdirectory)
            # Check if it directly contains any trace files
            has_trace_files = False
            for sub_item_name in io.listdir(item_path):
                if not io.isdir(io.join(item_path, sub_item_name)) and utils.is_chrome_trace_file(sub_item_name):
                    has_trace_files = True
                    break
            if has_trace_files:
                runs.add(item_name) # Add the subdirectory name

    sorted_runs = sorted(list(runs))
    logger.info(f"Found runs in {logdir}: {sorted_runs}")
    return sorted_runs
